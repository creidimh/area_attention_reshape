...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_area_flash_attention.h"
...
int main()
{
    ...
    // 4. 调用aclnnAreaFlashAttention第一段接口
    ret = aclnnAreaFlashAttentionGetWorkspaceSize(selfX, selfY, out, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAreaFlashAttentionGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 5. 调用aclnnAreaFlashAttention第二段接口
    ret = aclnnAreaFlashAttention(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnAreaFlashAttention failed. ERROR: %d\n", ret); return ret);
